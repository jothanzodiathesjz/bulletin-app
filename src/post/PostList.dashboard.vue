    ...
    >
      ...
      <tr v-for="(post, index) in result.data" class="table-row-custom">
        <td class="table-cell-custom">
          {{ ((result.current_page ?? 0) - 1) * (result.per_page ?? 0) +  (index + 1)}}
        </td>
        <td class="table-cell-custom">
          {{
...
import { type PostWithCommentsAndTags, type PostResponseArray } from "@/domain/Post";
